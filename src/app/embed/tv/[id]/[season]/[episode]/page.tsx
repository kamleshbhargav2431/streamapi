...
export default function EmbedTV() {
  ...
  const epTitle = `S${season}E${episode}`;

  const fetchSources = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      // First get the show title from TMDB
      const showRes = await fetch(`/api/tmdb?action=tv&id=${showId}`);
      if (!showRes.ok) throw new Error('Show not found');
      const showData = await showRes.json();
      const title = showData.name || '';
      const year = (showData.first_air_date || '').split('-')[0];

      const res = await fetch(`/api/sources?title=${encodeURIComponent(title)}&mediaType=tv&year=${year}&tmdbId=${showId}&season=${season}&episode=${episode}`);
        ...
        {loading ? (
          <div className="aspect-video w-full max-w-[1280px] bg-white/5 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">Loading {epTitle}...</p>
            </div>
          </div>
        ) : currentSources.length > 0 ? (
          <div className="w-full max-w-[1280px]">
            <VideoPlayer sources={currentSources} subtitles={currentSubs} title={epTitle} />
          </div>
        ) : (
          <div className="aspect-video w-full max-w-[1280px] bg-white/5 flex items-center justify-center">
            <div className="text-center">
